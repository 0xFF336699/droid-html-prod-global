 ...
 * @returns 包含所有脚本的地图
 */ async function fetchExamplesMap() {
    let lang = arguments.length > 0 && arguments[0] !== void 0 ? arguments[0] : 'zh-CN', withScript = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : false;
...
;// ./src/hooks/useDroidDocsScripts.ts
/**
 ...
 * @param lang 语言代码，默认为 'zh-CN'
 * @returns 脚本数据和操作方法
 */ function useDroidDocsScripts() {
    let lang = arguments.length > 0 && arguments[0] !== void 0 ? arguments[0] : 'zh-CN';
    // 脚本地图状态
        ...
        const loadMap = async ()=>{
            ...
            try {
                ...
                if (map.beginner && map.beginner.length > 0) {
                    console.log('[useDroidDocsScripts] 自动选择初级的第一个脚本:', map.beginner[0].name);
                    setSelectedScript(map.beginner[0]);
                    setScriptContent(map.beginner[0].script);
                }
            } catch (error) {
    ...
    const setSelectedLevel = (0,react.useCallback)((level)=>{
        ...
        if (examplesMap && examplesMap[level] && examplesMap[level].length > 0) {
            console.log('[useDroidDocsScripts] 自动选择新级别的第一个脚本:', examplesMap[level][0].name);
            setSelectedScript(examplesMap[level][0]);
            setScriptContent(examplesMap[level][0].script);
        } else {
...
function ScriptEditorContent(param) {
    ...
    const { examplesMap, selectedLevel, selectedScript, scriptContent, isLoadingMap, isLoadingScript, mapError, scriptError, setSelectedLevel, selectScript, updateScriptContent } = useDroidDocsScripts('zh-CN');
...
/***/ ((__unused_webpack_module, __unused_webpack_exports, __webpack_require__) => {
    ...
    (window.__NEXT_P = window.__NEXT_P || []).push([
      "/script-creator",
      function () {
        return __webpack_require__(5774);
      }
    ]);
    if(false) {}
