...
var project_v2_ext_src = __webpack_require__(64205);
...
/* harmony import */ var _barrel_optimize_names_Button_Dialog_DialogActions_DialogContent_DialogTitle_mui_material__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(46822);
/* harmony import */ var _barrel_optimize_names_Button_Dialog_DialogActions_DialogContent_DialogTitle_mui_material__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(16880);
/* harmony import */ var _barrel_optimize_names_Button_Dialog_DialogActions_DialogContent_DialogTitle_mui_material__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(88149);
/* harmony import */ var _barrel_optimize_names_Button_Dialog_DialogActions_DialogContent_DialogTitle_mui_material__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(89701);
/* harmony import */ var _barrel_optimize_names_Button_Dialog_DialogActions_DialogContent_DialogTitle_mui_material__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__(20872);
/* harmony import */ var react__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(94285);
/* harmony import */ var _barrel_optimize_names_useTranslation_react_i18next__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(51011);
...
/***/ ((__unused_webpack_module, __unused_webpack_exports, __webpack_require__) => {
    ...
    (window.__NEXT_P = window.__NEXT_P || []).push([
      "/project-editor",
      function () {
        return __webpack_require__(50880);
      }
    ]);
    if(false) {}
...
;// ../../libs/app/model/src/remote/project/upsertProject.ts


/**
 * 创建或更新项目（合并接口）
 * uuid 为空字符串或不传表示创建，有值表示更新
 * @param opts 请求选项
 * @returns 操作结果
 */ async function upsertProject(opts) {
    const isCreate = !opts.params.data.uuid;
    const operationType = isCreate ? '创建' : '更新';
    console.log('[upsertProject] 开始' + operationType + '项目, params:', JSON.stringify(opts.params));
    const res = await (0,request/* request */.Em)(tms/* tms */.X.TProject, tms/* tms */.X.MProjectUpsertProject, opts.params, opts.gateway, opts.handler);
    if (res.shellIsOk) {
        console.log('[upsertProject] 请求成功, shellIsOk:', res.shellIsOk);
    } else {
        console.log('[upsertProject] 请求失败, shellIsOk:', res.shellIsOk, 'err:', res.err);
    }
    if (res.dataIsOk) {
        console.log('[upsertProject] 数据正常, dataIsOk:', res.dataIsOk);
    } else {
        var _res_data, _res_data1;
        console.log('[upsertProject] 数据异常, dataIsOk:', res.dataIsOk, 'errMsg:', (_res_data = res.data) === null || _res_data === void 0 ? void 0 : _res_data.errMsg, 'errCode:', (_res_data1 = res.data) === null || _res_data1 === void 0 ? void 0 : _res_data1.errCode);
    }
    if (res.data) {
        if (res.data.is_ok) {
            var _res_data_project;
            const resultType = res.data.isCreated ? '创建' : '更新';
            console.log('[upsertProject] ' + resultType + '成功, project.uuid:', (_res_data_project = res.data.project) === null || _res_data_project === void 0 ? void 0 : _res_data_project.uuid);
            if (res.data.changes) {
                console.log('[upsertProject] 变更信息:', JSON.stringify(res.data.changes));
            }
            if (res.data.deletedChildren) {
                console.log('[upsertProject] 删除的子级:', JSON.stringify(res.data.deletedChildren));
            }
            if (res.data.createdChildren) {
                console.log('[upsertProject] 创建的子级:', JSON.stringify(res.data.createdChildren));
            }
        } else {
            console.log('[upsertProject] ' + operationType + '失败, msg:', res.data.msg);
        }
    } else {
        console.log('[upsertProject] 无返回数据');
    }
    return res;
}
/* harmony default export */ const project_upsertProject = ((/* unused pure expression or super */ null && (upsertProject)));
 ...
 */ function syncUuidsFromServer(localProject, serverProject) {
    ...
    const draftProject = src.projectStore.getDraft(ulid);
    ...
    const listProject = src.projectStore.findSavedProject(ulid);
    ...
    const historyList = (0,src.getProjectHistory)(ulid);
 ...
 */ async function saveToServer(project) {
    ...
    if (settings) {
        settings.remoteSyncStatus = src.RemoteSyncStatus.Pending;
    }
    try {
        var _res_data;
        ...
        if (res.dataIsOk && ((_res_data = res.data) === null || _res_data === void 0 ? void 0 : _res_data.is_ok) && res.data.project) {
            ...
            // 更新状态
            if (settings) {
                settings.remoteSyncStatus = src.RemoteSyncStatus.Synced;
            ...
            if (settings) {
                settings.remoteSyncStatus = src.RemoteSyncStatus.Failed;
            }
...
const editProjectInfoUtils = {
    ...
    getEditInfoByUlid: (ulid)=>{
        ...
        const project = src.projectStore.findProject(ulid);
 ...
 */ const defaultUIConfig = src.PROJECT_CATEGORY_CONFIGS[src.ProjectCategory.Workflow].uiConfig;
/**
 ...
 */ function createNodeTriggerActionPair(pn, an, cn, node) {
    ...
    // 获取 trigger utils
    const triggerUtils = (0,src.getTriggerUtils)(project_v2_ext_src.triggerNodeTriggerType);
    nodeClickUtils_fileLog.log('[createNodeTriggerActionPair] triggerUtils for type:', project_v2_ext_src.triggerNodeTriggerType, 'found:', !!triggerUtils);
    if (!triggerUtils) {
        throw new Error("[createNodeTriggerActionPair] triggerUtils not found for type: ".concat(project_v2_ext_src.triggerNodeTriggerType));
    }
    ...
    // 获取 action utils
    const actionUtils = (0,src.getActionUtils)(project_v2_ext_src.actionNodeActionType);
    nodeClickUtils_fileLog.log('[createNodeTriggerActionPair] actionUtils for type:', project_v2_ext_src.actionNodeActionType, 'found:', !!actionUtils);
    if (!actionUtils) {
        throw new Error("[createNodeTriggerActionPair] actionUtils not found for type: ".concat(project_v2_ext_src.actionNodeActionType));
    }
    // 使用 mergeDataToDefaultData 创建 action 数据
    const actionUlid = (0,index_esm/* ulid */.Z0)();
    const actionJavaData = {
        actionType: project_v2_ext_src.ActionType.click,
        clickConfig: {
    ...
    const actionGroup = src.actionGroupUtils.mergeDataToDefaultData({
        list: [
            actionData.ulid
        ],
        children: [
    ...
    const triggerGroup = src.triggerGroupUtils.mergeDataToDefaultData({
        list: [
            triggerData.ulid
        ],
        children: [
 ...
 */ function nodeToSelectorChain(node) {
    ...
    if (node.text && typeof node.text === 'string' && node.text.trim() !== '') {
        ...
        operations.push({
            method: project_v2_ext_src.SelectorOperation.TEXT,
            value: textValue
        });
    }
    ...
    if (node.className && typeof node.className === 'string') {
        ...
        operations.push({
            method: project_v2_ext_src.SelectorOperation.CLASS_NAME,
    ...
    if (!node.text || node.text.trim() === '') {
        ...
        if (node.contentDescription && typeof node.contentDescription === 'string' && node.contentDescription.trim() !== '') {
            ...
            operations.push({
                method: project_v2_ext_src.SelectorOperation.DESC,
        ...
        if (node.viewIdResourceName && typeof node.viewIdResourceName === 'string' && node.viewIdResourceName.trim() !== '') {
            ...
            operations.push({
                method: project_v2_ext_src.SelectorOperation.ID,
    ...
    const booleanAttrs = [
        {
            key: "isClickable",
            method: project_v2_ext_src.SelectorOperation.CLICKABLE
        },
        {
            key: "isEnabled",
            method: project_v2_ext_src.SelectorOperation.ENABLED
        },
        {
            key: "isVisibleToUser",
            method: project_v2_ext_src.SelectorOperation.VISIBLE_TO_USER
        },
        {
            key: "isScrollable",
            method: project_v2_ext_src.SelectorOperation.SCROLLABLE
        },
        {
            key: "isCheckable",
            method: project_v2_ext_src.SelectorOperation.CHECKABLE
        },
        {
            key: "isChecked",
            method: project_v2_ext_src.SelectorOperation.CHECKED
        },
        {
            key: "isFocusable",
            method: project_v2_ext_src.SelectorOperation.FOCUSABLE
        },
        {
            key: "isSelected",
            method: project_v2_ext_src.SelectorOperation.SELECTED
        }
    ];
    ...
    if (operations.length === 0 && node.boundsInScreen) {
        ...
        if (typeof bounds.left === 'number' && typeof bounds.top === 'number' && typeof bounds.right === 'number' && typeof bounds.bottom === 'number') {
            ...
            operations.push({
                method: project_v2_ext_src.SelectorOperation.BOUNDS,
                value: {
                    left: bounds.left,
                    top: bounds.top,
                    right: bounds.right,
...
var Add = __webpack_require__(62480);
...
function Action(param) {
    ...
    const { t } = (0,dist_es/* useTranslation */.Bd)('droid-project-v2-ui/ui/project/edit-project/trigger-action-group/trigger-action/action-group/action-list/action/content');
    const uiLog = Action_fileLog.sub(false, 'action_ui');
    uiLog.pause = true;
    uiLog.childrenPaused = true;
    const utils = (0,src.getActionUtils)(actionData.type);
    ...
    function openSettings() {
        var _editInfo_project;
        if ((editInfo === null || editInfo === void 0 ? void 0 : (_editInfo_project = editInfo.project) === null || _editInfo_project === void 0 ? void 0 : _editInfo_project.ulid) && (actionData === null || actionData === void 0 ? void 0 : actionData.ulid)) {
            router.push("/action-settings?projectUlid=".concat(editInfo.project.ulid, "&ulid=").concat(actionData.ulid));
        }
    }
    return /*#__PURE__*/ (0,jsx_runtime.jsxs)(Box/* default */.A, {
        sx: {
            ...
            /*#__PURE__*/ (0,jsx_runtime.jsxs)(Box/* default */.A, {
                ...
                children: [
                    /*#__PURE__*/ (0,jsx_runtime.jsx)(IconButton/* default */.A, {
                        ...
                        "data-testid": "Action__settingsButton__01JK2WHZZA1I8H5Y__".concat(index),
                        "data-test-comment": "设置按钮，点击后跳转至当前动作的详细设置页面",
                        children: /*#__PURE__*/ (0,jsx_runtime.jsx)(Settings/* default */.A, {
                            fontSize: "small"
                        })
                    }),
                    /*#__PURE__*/ (0,jsx_runtime.jsx)(ButtonConfirm/* ButtonConfirm */.b, {
                        title: t('Action.confirm.deleteTitle'),
                        content: t('Action.confirm.deleteContent'),
                        ...
                        children: /*#__PURE__*/ (0,jsx_runtime.jsx)(Button/* default */.A, {
                            ...
                            "data-test-comment": "删除动作按钮，点击后弹出二次确认对话框",
                            children: t('Action.action.delete')
                        })
                    })
                ]
            })
...
function ActionGroup(param) {
    ...
    const query = {
        ...
        actionGroupUlid: actionGroup.ulid,
        type: src.TriggerOrActionListTypeEnum.addAction,
        actionType: ''
    };
    ...
    function openSettings() {
        var _editInfo_project;
        if ((editInfo === null || editInfo === void 0 ? void 0 : (_editInfo_project = editInfo.project) === null || _editInfo_project === void 0 ? void 0 : _editInfo_project.ulid) && (actionGroup === null || actionGroup === void 0 ? void 0 : actionGroup.ulid)) {
            router.push("/action-group-settings?projectUlid=".concat(editInfo.project.ulid, "&ulid=").concat(actionGroup.ulid));
        }
    }
                            ...
                            uiConfig.showActionList && /*#__PURE__*/ (0,jsx_runtime.jsx)(Grid2/* default */.A, {
                                children: /*#__PURE__*/ (0,jsx_runtime.jsx)(Box/* default */.A, {
                                    children: /*#__PURE__*/ (0,jsx_runtime.jsx)(ActionListOrTriggerListPageLink, {
                                        query: query,
                                        attrs: (0,src.getActionGroupElement)(actionGroup),
                                        path: "/action-list",
...
function Trigger(param) {
    ...
    const { t } = (0,dist_es/* useTranslation */.Bd)('droid-project-v2-ui/ui/project/edit-project/trigger-action-group/trigger-action/trigger-group/trigger-list/trigger/content');
    const uiLog = Trigger_fileLog.sub(false, 'trigger_ui');
    uiLog.pause = true;
    uiLog.childrenPaused = true;
    const utils = (0,src.getTriggerUtils)(triggerData.type);
    ...
    function openSettings() {
        var _editInfo_project;
        if ((editInfo === null || editInfo === void 0 ? void 0 : (_editInfo_project = editInfo.project) === null || _editInfo_project === void 0 ? void 0 : _editInfo_project.ulid) && (triggerData === null || triggerData === void 0 ? void 0 : triggerData.ulid)) {
            router.push("/trigger-settings?projectUlid=".concat(editInfo.project.ulid, "&ulid=").concat(triggerData.ulid));
        }
    }
                ...
                /*#__PURE__*/ (0,jsx_runtime.jsxs)(Box/* default */.A, {
                    ...
                    children: [
                        /*#__PURE__*/ (0,jsx_runtime.jsx)(IconButton/* default */.A, {
                            size: "small",
                            onClick: openSettings,
                            "data-testid": "Trigger__settingsButton__01JK2WCXZA1H7H4Y__".concat(index),
                            "data-test-comment": "设置按钮，点击后跳转至当前触发器的详细设置页面",
                            children: /*#__PURE__*/ (0,jsx_runtime.jsx)(Settings/* default */.A, {
                                fontSize: "small"
                            })
                        }),
                        /*#__PURE__*/ (0,jsx_runtime.jsx)(ButtonConfirm/* ButtonConfirm */.b, {
                            title: t('Trigger.confirm.deleteTitle'),
                            content: t('Trigger.confirm.deleteContent'),
                            onConfirm: onDeleteComfirmed,
                            children: /*#__PURE__*/ (0,jsx_runtime.jsx)(Button/* default */.A, {
                                "data-testid": "Trigger__deleteButton__01JFXYZA7P1Q2R3S4T5U6V7W8__".concat(index),
                                id: "del-trigger-btn",
                                "data-id": "del-trigger-btn",
                                "data-test-comment": "删除触发器按钮，点击后弹出二次确认对话框",
                                children: t('Trigger.action.delete')
                            })
                        })
                    ]
                })
...
function TriggerGroup(param) {
    ...
    function openSettings() {
        var _editInfo_project;
        if ((editInfo === null || editInfo === void 0 ? void 0 : (_editInfo_project = editInfo.project) === null || _editInfo_project === void 0 ? void 0 : _editInfo_project.ulid) && (triggerGroup === null || triggerGroup === void 0 ? void 0 : triggerGroup.ulid)) {
            router.push("/trigger-group-settings?projectUlid=".concat(editInfo.project.ulid, "&ulid=").concat(triggerGroup.ulid));
        }
    }
                    ...
                    children: [
                        ...
                        /*#__PURE__*/ (0,jsx_runtime.jsx)(Grid2/* default */.A, {
                            children: /*#__PURE__*/ (0,jsx_runtime.jsx)(Box/* default */.A, {
                                children: /*#__PURE__*/ (0,jsx_runtime.jsx)(ActionListOrTriggerListPageLink, {
                                    query: query,
                                    attrs: src.childToProjectDataUtils.triggerGroup.getTriggerGroupElement(triggerGroup),
                                    path: "/trigger-list",
...
function TriggerAction(param) {
    ...
    function openSettings() {
        var _editInfo_project;
        const projectUlid = editInfo === null || editInfo === void 0 ? void 0 : (_editInfo_project = editInfo.project) === null || _editInfo_project === void 0 ? void 0 : _editInfo_project.ulid;
        if (projectUlid) {
            router.push("/trigger-action-settings?projectUlid=".concat(projectUlid, "&ulid=").concat(triggerAction.ulid));
        }
    }
...
function TriggerActionGroup(param) {
    ...
    const editInfo = (0,react.useContext)(ProjectEditInfoContext);
    const uiConfig = (0,react.useContext)(ProjectCategoryUIConfigContext);
    const contextTriggerActionGroup = (0,react.useContext)(src.TriggerActionGroupContext);
    const router = (0,next_router.useRouter)();
    ...
    function openSettings() {
        ...
        const projectUlid = editInfo === null || editInfo === void 0 ? void 0 : (_editInfo_project = editInfo.project) === null || _editInfo_project === void 0 ? void 0 : _editInfo_project.ulid;
        if (projectUlid) {
            router.push("/trigger-action-group-settings?projectUlid=".concat(projectUlid, "&ulid=").concat(triggerActionGroup === null || triggerActionGroup === void 0 ? void 0 : triggerActionGroup.ulid));
        }
    }
    ...
    function addTriggerAction() {
        ...
        const newTriggerAction = (0,src.createDefaultTriggerAction)();
    ...
    return /*#__PURE__*/ (0,jsx_runtime.jsx)(src.TriggerActionGroupContext.Provider, {
        ...
        children: /*#__PURE__*/ (0,jsx_runtime.jsxs)(Paper/* default */.A, {
            ...
            ...src.childToProjectDataUtils.triggerActionGroup.getTriggerActionGroupElementByProp(triggerActionGroup.list),
...
// EXTERNAL MODULE: ../../libs/droid/project-v2-ui/src/utils/runProject.ts
var runProject = __webpack_require__(89270);
...
function StayAlert() {
    ...
    function handleLeaveWithoutSave() {
        toLeave();
        src.projectStore.deleteDraft(editInfo.project.ulid);
    }
    ...
    function handleSaveToDraftAndLeave() {
        src.projectStore.addDraft(editInfo.project);
        toLeave();
    }
    return /*#__PURE__*/ (0,jsx_runtime.jsx)(Box/* default */.A, {
...
function EditProjectBuilder() {
    ...
    let project = src.projectStore.findDraftOrSavedProject(ulid || '-1');
    if (!project) {
        ...
        project = (0,src.createProjectByCategory)(src.ProjectCategory.Workflow);
    ...
    const draftProject = project = src.projectStore.addDraft(createProject);
    ...
    function checkUpdated() {
        ...
        logger.isPaused = true;
        const c = src.projectStore.findSavedProject(project.ulid);
    ...
    function Content() {
        (0,react.useEffect)(()=>{
            ...
            (0,src.takeProjectSnapshoot)(project);
            if (!project) return;
        ...
        (0,react.useEffect)(()=>{
            const unsub = jsBridgeDispatcher.addListener("checkBrowserCanBack", (event)=>{
                ...
                if (canBack) src.projectStore.deleteDraft(project.ulid);
            });
            return ()=>{
                unsub();
            };
        ...
        (0,react.useEffect)(()=>{
            ...
            const isQuickAction = project.category === src.ProjectCategory.QuickAction;
        ...
        // 获取项目类别的 UI 配置
        const categoryConfig = (0,src.getProjectCategoryConfig)(editInfo.project.category);
...
;// ../../libs/app/model/src/remote/project/getProjectR2Url.ts



const getProjectR2Url_fileLog = new Log/* Log */.tG(false, 'getProjectR2Url_f');
/**
 * 获取项目在R2的project.json完整URL
 * 仅支持public且未删除的项目
 * @param opts 请求选项
 * @returns R2 URL
 */ async function getProjectR2Url(opts) {
    const fnLog = getProjectR2Url_fileLog.sub(false, 'getProjectR2Url_fn');
    // fnLog.pause = true;
    fnLog.log('[getProjectR2Url] 开始获取项目R2 URL, params:', JSON.stringify(opts.params));
    const res = await (0,request/* request */.Em)(tms/* tms */.X.TProject, tms/* tms */.X.MProjectGetProjectR2Url, opts.params, opts.gateway, opts.handler);
    fnLog.log('[getProjectR2Url] 请求完成, shellIsOk:', res.shellIsOk, 'dataIsOk:', res.dataIsOk);
    if (!res.shellIsOk) {
        fnLog.log('[getProjectR2Url] ERROR: 请求失败, err:', JSON.stringify(res.err));
    }
    if (!res.dataIsOk) {
        var _res_data, _res_data1;
        fnLog.log('[getProjectR2Url] ERROR: 数据异常, errMsg:', (_res_data = res.data) === null || _res_data === void 0 ? void 0 : _res_data.errMsg, 'errCode:', (_res_data1 = res.data) === null || _res_data1 === void 0 ? void 0 : _res_data1.errCode);
    }
    if (res.data) {
        if (res.data.is_ok) {
            fnLog.log('[getProjectR2Url] 获取成功, url:', res.data.url);
        } else {
            fnLog.log('[getProjectR2Url] ERROR: 获取失败, msg:', res.data.msg);
        }
    } else {
        fnLog.log('[getProjectR2Url] ERROR: 无返回数据');
    }
    return res;
}
/* harmony default export */ const project_getProjectR2Url = ((/* unused pure expression or super */ null && (getProjectR2Url)));

;// ../../libs/droid/project-v2-ui/src/ui/project/edit-project/MarketModeWrapper.tsx
...
const MarketModeWrapper_fileLog = new Log/* Log */.tG(false, 'MarketModeWrapper_f');
MarketModeWrapper_fileLog.pause = true;
MarketModeWrapper_fileLog.childrenPaused = true;
var MarketModeLoadingState = /*#__PURE__*/ function(MarketModeLoadingState) {
    MarketModeLoadingState["Idle"] = "idle";
    MarketModeLoadingState["LoadingUrl"] = "loadingUrl";
    MarketModeLoadingState["LoadingData"] = "loadingData";
    MarketModeLoadingState["Success"] = "success";
    MarketModeLoadingState["Error"] = "error";
    return MarketModeLoadingState;
}({});
/**
 * 重新生成 project 及所有子孙级的 ulid
 * 用于从市场加载项目时，创建独立的本地副本
 */ function regenerateAllUlids(project, log) {
    const fnLog = log.sub(false, 'regenerateAllUlids_fn');
    const oldProjectUlid = project.ulid;
    project.ulid = (0,index_esm/* ulid */.Z0)();
    fnLog.log('[regenerateAllUlids] project.ulid:', oldProjectUlid, '->', project.ulid);
    if (!project.children) {
        fnLog.log('[regenerateAllUlids] project.children 为空，跳过');
        return;
    }
    for(let i = 0; i < project.children.length; i++){
        const triggerActionGroup = project.children[i];
        const oldTagUlid = triggerActionGroup.ulid;
        triggerActionGroup.ulid = (0,index_esm/* ulid */.Z0)();
        fnLog.log('[regenerateAllUlids] triggerActionGroup[' + i + '].ulid:', oldTagUlid, '->', triggerActionGroup.ulid);
        if (!triggerActionGroup.children) {
            fnLog.log('[regenerateAllUlids] triggerActionGroup[' + i + '].children 为空，跳过');
            continue;
        }
        for(let j = 0; j < triggerActionGroup.children.length; j++){
            const triggerAction = triggerActionGroup.children[j];
            const oldTaUlid = triggerAction.ulid;
            triggerAction.ulid = (0,index_esm/* ulid */.Z0)();
            fnLog.log('[regenerateAllUlids] triggerAction[' + i + '][' + j + '].ulid:', oldTaUlid, '->', triggerAction.ulid);
            // 处理 triggerGroup
            if (triggerAction.triggerGroup) {
                const oldTgUlid = triggerAction.triggerGroup.ulid;
                triggerAction.triggerGroup.ulid = (0,index_esm/* ulid */.Z0)();
                triggerAction.triggerGroupUlid = triggerAction.triggerGroup.ulid;
                fnLog.log('[regenerateAllUlids] triggerGroup[' + i + '][' + j + '].ulid:', oldTgUlid, '->', triggerAction.triggerGroup.ulid);
                if (triggerAction.triggerGroup.children) {
                    for(let k = 0; k < triggerAction.triggerGroup.children.length; k++){
                        const trigger = triggerAction.triggerGroup.children[k];
                        const oldTriggerUlid = trigger.ulid;
                        trigger.ulid = (0,index_esm/* ulid */.Z0)();
                        fnLog.log('[regenerateAllUlids] trigger[' + i + '][' + j + '][' + k + '].ulid:', oldTriggerUlid, '->', trigger.ulid);
                    }
                }
            }
            // 处理 actionGroup
            if (triggerAction.actionGroup) {
                const oldAgUlid = triggerAction.actionGroup.ulid;
                triggerAction.actionGroup.ulid = (0,index_esm/* ulid */.Z0)();
                triggerAction.actionGroupUlid = triggerAction.actionGroup.ulid;
                fnLog.log('[regenerateAllUlids] actionGroup[' + i + '][' + j + '].ulid:', oldAgUlid, '->', triggerAction.actionGroup.ulid);
                if (triggerAction.actionGroup.children) {
                    for(let k = 0; k < triggerAction.actionGroup.children.length; k++){
                        const action = triggerAction.actionGroup.children[k];
                        const oldActionUlid = action.ulid;
                        action.ulid = (0,index_esm/* ulid */.Z0)();
                        fnLog.log('[regenerateAllUlids] action[' + i + '][' + j + '][' + k + '].ulid:', oldActionUlid, '->', action.ulid);
                    }
                }
            }
        }
    }
    fnLog.log('[regenerateAllUlids] 完成');
}
function MarketModeWrapper(param) {
    let { children } = param;
    const uiLog = MarketModeWrapper_fileLog.sub(false, 'MarketModeWrapper_ui');
    const router = (0,next_router.useRouter)();
    const { t } = (0,dist_es/* useTranslation */.Bd)('droid-project-v2-ui/ui/project/edit-project/content');
    const [loadingState, setLoadingState] = (0,react.useState)("idle");
    const [errorMsg, setErrorMsg] = (0,react.useState)('');
    const mode = router.query.mode;
    const uuid = router.query.uuid;
    const urlUlid = router.query.ulid;
    const isMarketMode = mode === 'market';
    uiLog.log('[MarketModeWrapper] mode:', mode, 'uuid:', uuid, 'urlUlid:', urlUlid, 'isMarketMode:', isMarketMode);
    (0,react.useEffect)(()=>{
        const fnLog = uiLog.sub(false, 'effect_loadMarketProject_fn');
        fnLog.log('[loadMarketProject] isMarketMode:', isMarketMode, 'uuid:', uuid, 'urlUlid:', urlUlid);
        if (!isMarketMode) {
            return;
        }
        if (!uuid) {
            setLoadingState("error");
            setErrorMsg(t('MarketModeWrapper.error.missingUuid'));
            return;
        }
        // 如果 URL 已有 ulid，说明已经加载过，检查本地是否存在
        if (urlUlid) {
            const existingByUlid = src.projectStore.findDraftOrSavedProject(urlUlid);
            fnLog.log('[loadMarketProject] URL 有 ulid, 检查本地, existingByUlid:', existingByUlid ? existingByUlid.ulid : 'null');
            if (existingByUlid) {
                setLoadingState("success");
                return;
            }
        }
        loadMarketProject(uuid, fnLog, t, router);
    }, [
        isMarketMode,
        uuid,
        urlUlid,
    ...
    async function loadMarketProject(uuid, log, t, router) {
        var _r2UrlResult_data, _r2UrlResult_data1;
        setLoadingState("loadingUrl");
        setErrorMsg('');
        // 1. 获取 R2 URL
        const r2UrlResult = await getProjectR2Url({
            params: {
                data: {
                    uuid
                }
        ...
        log.log('[loadMarketProject] getProjectR2Url 结果, shellIsOk:', r2UrlResult.shellIsOk, 'dataIsOk:', r2UrlResult.dataIsOk, 'is_ok:', (_r2UrlResult_data = r2UrlResult.data) === null || _r2UrlResult_data === void 0 ? void 0 : _r2UrlResult_data.is_ok, 'url:', (_r2UrlResult_data1 = r2UrlResult.data) === null || _r2UrlResult_data1 === void 0 ? void 0 : _r2UrlResult_data1.url);
        if (!r2UrlResult.shellIsOk || !r2UrlResult.data) {
            setLoadingState("error");
            setErrorMsg(t('MarketModeWrapper.error.fetchUrlFailed'));
            return;
        }
        if (!r2UrlResult.data.is_ok) {
            setLoadingState("error");
            const msg = r2UrlResult.data.msg;
            if (msg) {
                setErrorMsg(msg);
            } else {
                setErrorMsg(t('MarketModeWrapper.error.projectNotExist'));
            }
            return;
        }
        const r2Url = r2UrlResult.data.url;
        if (!r2Url) {
            setLoadingState("error");
            setErrorMsg(t('MarketModeWrapper.error.urlEmpty'));
            return;
        }
        // 2. Fetch 项目数据
        ...
        try {
            ...
            log.log('[loadMarketProject] fetch 响应, ok:', response.ok, 'status:', response.status);
            if (!response.ok) {
                setLoadingState("error");
                setErrorMsg(t('MarketModeWrapper.error.fetchDataFailed'));
                return;
            }
            const projectData = await response.json();
            log.log('[loadMarketProject] 获取到项目数据, title:', projectData.title, 'uuid:', projectData.uuid);
            // 3. 重新生成所有 ulid（创建独立的本地副本）
            regenerateAllUlids(projectData, log);
            // 4. 存入 draft
            src.projectStore.addDraft(projectData);
            log.log('[loadMarketProject] 已存入 draft, ulid:', projectData.ulid);
            // 5. 更新 URL 添加 ulid 参数，让 EditProjectBuilder 能找到项目
            const currentQuery = {
                ...router.query
            };
            currentQuery.ulid = projectData.ulid;
            log.log('[loadMarketProject] 更新 URL, 添加 ulid:', projectData.ulid);
            router.replace({
            ...
            log.log('[loadMarketProject] ERROR: fetch 异常:', error);
            setLoadingState("error");
            setErrorMsg(t('MarketModeWrapper.error.fetchDataFailed'));
        }
    }
    function handleRetry() {
        const fnLog = uiLog.sub(false, 'handleRetry_fn');
        fnLog.log('[handleRetry] 重试加载, uuid:', uuid);
        if (uuid) {
            loadMarketProject(uuid, fnLog, t, router);
        }
    }
    function handleBack() {
        ...
        fnLog.log('[handleBack] 返回市场页面');
        router.push('/fanfanlo/market');
    }
    // 非市场模式，直接渲染子组件
    if (!isMarketMode) {
        return /*#__PURE__*/ (0,jsx_runtime.jsx)(jsx_runtime.Fragment, {
    ...
    // 加载中状态
    const isLoadingState = loadingState === "idle" || loadingState === "loadingUrl" || loadingState === "loadingData";
    if (isLoadingState) {
        return /*#__PURE__*/ (0,jsx_runtime.jsx)(MarketModeLoadingUI, {
            loadingState: loadingState
        });
    }
    // 错误状态
    if (loadingState === "error") {
        return /*#__PURE__*/ (0,jsx_runtime.jsx)(MarketModeErrorUI, {
            errorMsg: errorMsg,
            onBack: handleBack,
            onRetry: handleRetry
        });
...
function MarketModeLoadingUI(param) {
    let { loadingState } = param;
    const uiLog = MarketModeWrapper_fileLog.sub(false, 'MarketModeLoadingUI_ui');
    const { t } = (0,dist_es/* useTranslation */.Bd)('droid-project-v2-ui/ui/project/edit-project/content');
    let loadingText = t('MarketModeWrapper.loading.default');
    if (loadingState === "loadingUrl") {
        loadingText = t('MarketModeWrapper.loading.url');
    } else if (loadingState === "loadingData") {
        loadingText = t('MarketModeWrapper.loading.data');
    }
    uiLog.log('[MarketModeLoadingUI] loadingState:', loadingState, 'loadingText:', loadingText);
    return /*#__PURE__*/ (0,jsx_runtime.jsxs)(Box/* default */.A, {
        sx: {
            display: 'flex',
        ...
        "data-testid": "MarketModeWrapper__loading__01jw9a1b2c3d4e5f6g7h8i9j",
        "data-test-comment": "市场模式加载中状态",
        children: [
...
function MarketModeErrorUI(param) {
    let { errorMsg, onBack, onRetry } = param;
    const uiLog = MarketModeWrapper_fileLog.sub(false, 'MarketModeErrorUI_ui');
    const { t } = (0,dist_es/* useTranslation */.Bd)('droid-project-v2-ui/ui/project/edit-project/content');
    uiLog.log('[MarketModeErrorUI] errorMsg:', errorMsg);
    return /*#__PURE__*/ (0,jsx_runtime.jsxs)(Box/* default */.A, {
        sx: {
            display: 'flex',
            flexDirection: 'column',
        ...
        "data-testid": "MarketModeWrapper__error__01jw9a1b2c3d4e5f6g7h8i9k",
        "data-test-comment": "市场模式错误状态",
        children: [
            /*#__PURE__*/ (0,jsx_runtime.jsx)(Typography/* default */.A, {
                color: "error",
                children: errorMsg
            ...
            /*#__PURE__*/ (0,jsx_runtime.jsxs)(Box/* default */.A, {
                ...
                children: [
                    /*#__PURE__*/ (0,jsx_runtime.jsx)(Button/* default */.A, {
                        variant: "outlined",
                        onClick: onBack,
                        "data-testid": "MarketModeWrapper__backButton__01jw9a1b2c3d4e5f6g7h8i9l",
                        "data-test-comment": "返回市场页面按钮",
                        children: t('MarketModeWrapper.error.back')
                    }),
                    /*#__PURE__*/ (0,jsx_runtime.jsx)(Button/* default */.A, {
                        variant: "contained",
                        onClick: onRetry,
                        "data-testid": "MarketModeWrapper__retryButton__01jw9a1b2c3d4e5f6g7h8i9m",
                        "data-test-comment": "重试按钮",
                        children: t('MarketModeWrapper.error.retry')
                    })
                ]
            })
        ]
...
// EXTERNAL MODULE: ./src/components/page/action-list/action.data.ts + 11 modules
var action_data = __webpack_require__(52358);
...
function ProjectEditorV2Test() {
    ...
    return /*#__PURE__*/ (0,jsx_runtime.jsx)(AppShell/* AppShell */.G, {
        children: /*#__PURE__*/ (0,jsx_runtime.jsx)(AppBar/* AppbarContainer */.W, {
            ...
            children: /*#__PURE__*/ (0,jsx_runtime.jsx)(MarketModeWrapper, {
...
"use strict";
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   t: () => (/* binding */ runProject)
/* harmony export */ });
/* harmony import */ var _droid_android__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(68709);
/* harmony import */ var _droid_project_v2_ext__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(64205);
/* harmony import */ var _fanfanlo__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(89597);
/* harmony import */ var _droid_project_v2_interface__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(34456);
/* harmony import */ var _app_model__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(80530);





const fileLog = new _fanfanlo__WEBPACK_IMPORTED_MODULE_2__/* .Log */ .tG(false, 'runProject_f');
fileLog.pause = false;
 ...
 */ async function runProject(projectData) {
    const fnLog = fileLog.sub(false, 'runProject_fn');
    fnLog.pause = false;
    fnLog.childrenPaused = false;
    ...
    const removedCount = _droid_project_v2_ext__WEBPACK_IMPORTED_MODULE_0__.projectUtils.cleanEmptyTriggerActions(projectData);
    ...
    try {
        ...
        const permissions = await _droid_project_v2_ext__WEBPACK_IMPORTED_MODULE_0__.projectUtils.getProjectPermissionsData(projectData);
        fnLog.log('[runProject] 权限数量:', permissions.length);
        // 2. 获取项目所需的注入配置
        fnLog.log('[runProject] 获取注入配置', JSON.stringify(projectData));
        const projectInjects = _droid_project_v2_ext__WEBPACK_IMPORTED_MODULE_0__.projectUtils.getProjectInjects(projectData);
        ...
        const finalInjects = [
            [
                _droid_project_v2_interface__WEBPACK_IMPORTED_MODULE_1__.WebViewJsInjectKeys.console,
                'console'
            ]
        ];
        ...
        for(let i = 0; i < projectInjects.length; i++){
            ...
            const isInjectNull = inject === null;
            const isInjectUndefined = inject === undefined;
            fnLog.log('[runProject] 处理inject[', i, ']:', inject);
            fnLog.log('[runProject] inject是否为null:', isInjectNull);
            fnLog.log('[runProject] inject是否为undefined:', isInjectUndefined);
            if (isInjectNull) {
                fnLog.log('[runProject] 警告: inject[', i, '] 为null, 跳过');
                continue;
            }
            if (isInjectUndefined) {
                fnLog.log('[runProject] 警告: inject[', i, '] 为undefined, 跳过');
                continue;
            }
            finalInjects.push(inject);
        }
        fnLog.log('[runProject] 最终injects数量:', finalInjects.length);
        fnLog.log('[runProject] 最终injects内容:', JSON.stringify(finalInjects));
        fnLog.log('[runProject] 检查finalInjects中的每个元素');
        for(let i = 0; i < finalInjects.length; i++){
            const inject = finalInjects[i];
            const isNull = inject === null;
            const isUndefined = inject === undefined;
            const isArray = Array.isArray(inject);
            fnLog.log('[runProject] finalInjects[', i, '] 是否为null:', isNull);
            fnLog.log('[runProject] finalInjects[', i, '] 是否为undefined:', isUndefined);
            fnLog.log('[runProject] finalInjects[', i, '] 是否为数组:', isArray);
            if (isArray) {
                const arrayLength = inject.length;
                fnLog.log('[runProject] finalInjects[', i, '] 数组长度:', arrayLength);
                if (arrayLength > 0) {
                    fnLog.log('[runProject] finalInjects[', i, '][0]:', inject[0]);
                } else {
                    fnLog.log('[runProject] finalInjects[', i, '] 是空数组');
                }
            } else {
                fnLog.log('[runProject] finalInjects[', i, '] 内容:', inject);
            }
        }
        // 4. 组装 RhinoRunner 配置
        fnLog.log('[runProject] 组装RhinoRunner配置');
        const conf = (0,_droid_project_v2_interface__WEBPACK_IMPORTED_MODULE_1__.assembleRhinoRunner)(projectData, permissions, {
        ...
        const userId = _app_model__WEBPACK_IMPORTED_MODULE_3__/* .user */ .k.data.storeData.user_token || '';
        fnLog.log('[runProject] userId:', userId);
        const projectName = projectData.name || 'unnamed_project';
        const projectUlid = projectData.ulid;
        fnLog.log('[runProject] projectName:', projectName);
        fnLog.log('[runProject] projectUlid:', projectUlid);
        const result = await _droid_android__WEBPACK_IMPORTED_MODULE_4__/* .autoWebViewJs */ .yx.runProject(conf, userId, projectName, projectUlid);
...
/******/ __webpack_require__.O(0, [7772,472,402,5241,6940,1490,1322,5409,2314,8143,9050,4550,8547,5475,9815,4571,4706,1508,1469,4456,4100,4205,1562,2358,636,6593,8792], () => (__webpack_exec__(40373)));
/******/ var __webpack_exports__ = __webpack_require__.O();
/******/ _N_E = __webpack_exports__;
/******/ }
]);
//# sourceMappingURL=project-editor-8fb5899aa15fca11.js.map