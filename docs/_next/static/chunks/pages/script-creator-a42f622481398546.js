...
/***/ ((__unused_webpack_module, __unused_webpack_exports, __webpack_require__) => {
    ...
    (window.__NEXT_P = window.__NEXT_P || []).push([
      "/script-creator",
      function () {
        return __webpack_require__(80873);
      }
    ]);
    if(false) {}
...
;// ./src/components/script-editor/content/scripts/AdjustVolume.ts
const adjustVolume = {
    name: 'AdjustVolume',
    description: '调节音量示例',
...
;// ./src/components/script-editor/content/scripts/VibratePhone.ts
const vibratePhone = {
    name: 'VibratePhone',
    description: '手机震动示例',
...
;// ./src/components/script-editor/content/scripts/SendNotification.ts
...
;// ./src/components/script-editor/content/scripts/index.ts
...
function ScriptEditorContent(param) {
    let { ns } = param;
    // 默认选择 SendNotification（索引2）
    const [selectedScript, setSelectedScript] = (0,react.useState)(scriptList[2]);
    const [scriptContent, setScriptContent] = (0,react.useState)(scriptList[2].script);
    ...
    // 使用 useRef 标记是否正在加载，防止重复加载
    const isLoadingRef = (0,react.useRef)(false);
    // 使用 useEffect 异步加载数据，防止无限渲染
    (0,react.useEffect)(()=>{
        // 防止重复加载
        if (isLoadingRef.current) {
            console.log("[ScriptEditorContent] 数据已在加载中，跳过");
            return;
        }
        console.log("[ScriptEditorContent] 开始加载数据，ns=".concat(ns));
        isLoadingRef.current = true;
        let isMounted = true;
        loadScrptEditorData(ns).then((result)=>{
        ...
        return ()=>{
            console.log("[ScriptEditorContent] useEffect cleanup");
            isMounted = false;
        };
    }, [
        ns
    ]);
    // 创建稳定的加载函数引用，仅当 ns 改变时重新创建
    // 但不立即执行，而是让 ContextLoader 来执行
    const loadData = (0,react.useCallback)(()=>{
        console.log("[ScriptEditorContent] loadData 函数被调用");
        // 直接返回已加载的数据或执行加载
        if (contextData) {
            return Promise.resolve(contextData);
        }
        return loadScrptEditorData(ns);
    }, [
        contextData,
        ns
    ]);
    const handleScriptChange = (scriptName)=>{
        console.log("[ScriptEditorContent] 选择脚本:", scriptName);
        const selected = scriptMap[scriptName];
        if (selected) {
            console.log("[ScriptEditorContent] 选择脚本: 找到了", scriptName);
            setSelectedScript(selected);
            setScriptContent(selected.script);
        } else {
            console.log("[ScriptEditorContent] 选择脚本: 没找到", scriptName);
        }
    };
    function onRunClick() {
        console.log("[ScriptEditorContent] 执行脚本,脚本名称:".concat(selectedScript.name));
        setRunScript('');
                                ...
                                children: /*#__PURE__*/ (0,jsx_runtime.jsxs)(Stack/* default */.A, {
                                    ...
                                    children: [
                                        ...
                                        /*#__PURE__*/ (0,jsx_runtime.jsx)("select", {
                                            ...
                                            children: scriptList.map((script)=>/*#__PURE__*/ (0,jsx_runtime.jsxs)("option", {
                                                    value: script.name,
                                                    children: [
                                                        script.name,
                                                        " - ",
...
/******/ __webpack_require__.O(0, [7054,1482,1174,3716,636,6593,8792], () => (__webpack_exec__(61350)));
/******/ var __webpack_exports__ = __webpack_require__.O();
/******/ _N_E = __webpack_exports__;
/******/ }
]);
//# sourceMappingURL=script-creator-a42f622481398546.js.map