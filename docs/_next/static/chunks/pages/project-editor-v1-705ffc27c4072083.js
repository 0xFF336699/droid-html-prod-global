(self["webpackChunk_N_E"] = self["webpackChunk_N_E"] || []).push([[2815],{
...
const editProjectInfoUtils = {
    edit: (project)=>{
        // const info = map[project.client_id] || (map[project.client_id] = createProjectEditInfo(project));
        const info = map[project.client_id] = createProjectEditInfo(project);
        return info;
    },
    getEditInfoByClientId: (clientId)=>{
        if (map[clientId]) return map[clientId];
        const project = storage/* projectStore */.D.findProject(clientId);
        if (project) return map[clientId] = createProjectEditInfo(project);
        return null;
    },
...
var ITriggerAction_intf = __webpack_require__(46764);
...
function ActionGroup(param) {
    ...
    const query = {
        projectClientId: editInfo.project.client_id,
...
function TriggerAction(param) {
    ...
    const uiConfig = (0,react.useContext)(ProjectCategoryUIConfigContext);
    TriggerAction_fileLog.log("[TriggerAction] uiConfig.showTrigger=".concat(uiConfig.showTrigger));
...
function TriggerActionGroup() {
    ...
    const [list] = (0,useWatchListOperation/* useWatchListOperation */.y)(editInfo.project.triggerActionGroup.list);
...
function ProjectContent() {
    ...
    return /*#__PURE__*/ (0,jsx_runtime.jsx)(Box/* default */.A, {
        "data-client-id-project": project.client_id,
...
function StayAlert() {
    ...
    function handleLeaveWithoutSave() {
        toLeave();
        storage/* projectStore */.D.deleteDraft(editInfo.project.client_id);
    }
...
function ProjectTools(param) {
    ...
    async function onRunClick() {
        ...
        fnLog.log('[onRunClick] projectData.client_id:', projectData.client_id);
...
function EditProjectBuilder() {
    ...
    let project = storage/* projectStore */.D.findDraftOrSavedProject(client_id || '-1') || (0,IProject_data/* mergeProjectDataToDefaultData */.u5)({
    ...
    function checkUpdated() {
        ...
        logger.isPaused = true;
        const c = storage/* projectStore */.D.findSavedProject(project.client_id);
    ...
    function Content() {
        ...
        (0,react.useEffect)(()=>{
            const unsub = jsbridge/* jsBridgeDispatcher */.y5.addListener("checkBrowserCanBack", (event)=>{
                ...
                if (canBack) storage/* projectStore */.D.deleteDraft(project.client_id);
            });
            return ()=>{
                unsub();
            };
        ...
        // 处理 QuickAction 新建时自动跳转到 action-list
        (0,react.useEffect)(()=>{
            ...
            if (isNew && isQuickAction) {
                ...
                // 构建 action-list 页面的查询参数
                const triggerAction = project.triggerActionGroup.list[0];
                if (!triggerAction) {
                    fnLog.log('[QuickAction Redirect] triggerAction 不存在，跳过跳转');
                    return;
                }
                // 如果 actionGroup 中已经有 action，说明已经选择过，不需要再跳转
                const hasAction = triggerAction.actionGroup.list.length > 0;
                fnLog.log('[QuickAction Redirect] hasAction:', hasAction);
                fnLog.log('[QuickAction Redirect] actionGroup.list.length:', triggerAction.actionGroup.list.length);
                if (hasAction) {
                ...
                const actionListQuery = {
                    ...
                    triggerActionGroupId: project.triggerActionGroup.id,
                    triggerActionId: triggerAction.id,
                    actionGroupId: triggerAction.actionGroup.id,
                    type: common/* ActionOrTriggerListTypeEnum */.Q.addAction,
                    actionType: '',
...
;// ./src/pages/project-editor-v1/index.tsx
